...
We'll like to test if the difference we observed between treatment and control mice was due to chance or not. So to do this, we test the Null hypothesis, which is that there's no significant difference and what we observed was due to chance, we'll use a null hypothesis distribution to test this, what that means is that 
...
We take new samples from the control population, and since we're testing the null hypothesis, we create new treatment and control samples, then we check the mean difference. We do this several times to get the null distribution, which is basically all possible observations under the null. 
...
Above, I've created the null distribution of the mice data, remember that the goal is to see how common it is to find the difference we observed with our experiment to see if its significant or not, so we test the null hypothesis, which is that the observation is not significant, to do this, we get random values from the control population to create a null distribution, which we then explore and can do statistical inference on  

```{r message=FALSE}
max(mice_null_distribution) # 5.12, which is actually bigger than the observation we saw in our experiment. Interesting.
...
A histogram is a useful plot use to explore the distribution, we see that most of the data lies between -2 and 2. This gives us a picture of how likely it is to observe the difference we did in our experiment. To compute how likely it is to find such observation as we did in our experiment, what we do is to calculate the proportion of times we find that our observation from the null distribution  
...
That 2.6% is the p-value. It is the probability that an outcome from the null distribution is bigger than what we observed when the null hypothesis is true. So this p-value is considered small enough to be reported (in scientific literatures). So why we can compute the null distribution in this example, we should note that this isn't always possible in real life because we usually don't get access to the full population from which our experimental sample is drawn, so we need to know how to compute these p-values without having access to the entire population.  

#### Probability Distributions  
Distributions of data gives us a number of information of the data we're working with. For example, given the distribution, we can tell the proportion below any point, and with that information, we can tell how many values are within any interval. So if we can describ the distribution, we can describe the entire list. Given a distribution, we can plot the **Emperical Cumulative Distribution Function**, which shows us the f(x) for any x, where x is the height, and f(x) is the proportion. Knowing the distribution of a population answers the question of what is the probability of picking a value randomly that matches a given spec.  Given this information, take for example we have a distribution of height, we can answer the question what is the probability of an individual being shorter than a given height.  
